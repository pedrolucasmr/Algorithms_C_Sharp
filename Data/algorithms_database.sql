...
USE algorithms;
CREATE TABLE runs(aName VARCHAR(255), timeNotation VARCHAR(255), spaceNotation VARCHAR(255),runStart TIMESTAMP,runEnd TIMESTAMP,totalTime TIMESTAMP,entries INT);

DELIMITER // 
CREATE PROCEDURE insertNewRun(IN iAName VARCHAR(255),IN iTimeNotation varchar(255),IN iSpaceNotation VARCHAR(255),IN iRunStart TIMESTAMP,IN iRunEnd TIMESTAMP,IN iTotalTime TIMESTAMP,IN iEntries INT)
BEGIN
